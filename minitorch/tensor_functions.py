...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # (1 - sigmoid) @ sigmoid @ grad = sigmoid @ grad - sigmoid @ sigmoid @ grad
        return a.f.add_zip(a.f.mul_zip(sigmoid, grad_output),
                           a.f.neg_map(a.f.mul_zip(a.f.mul_zip(sigmoid, sigmoid), grad_output)))
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        new_grad = grad_output._tensor.permute(*order)
        return minitorch.Tensor.make(new_grad._storage, new_grad.shape, new_grad.strides,
